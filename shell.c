...
typedef struct s_redir {
    int         type; 
    char        *target;
    int         fd;        // For heredoc pipe storage
    struct s_redir *next;
} t_redir;
...
int is_single_builtin(t_shell *shell) {
    if (!shell->cmd) return 0;
    if (shell->cmd->next) return 0;  // Pipeline exists
...
void execute_command_pipeline(t_shell *shell) {
    t_command *cmd = shell->cmd;
    int num_cmds = count_commands(shell);
    ...
    while (cmd) {
        ...
        if (pids[i] == 0) {  // Child process
            ...
            if (is_builtin(cmd->args[0])) {
                int ret = execute_builtin(shell, cmd->args);
                exit(ret);
            } else {
...
void execute_external(t_command *cmd, t_env *env) {
    char *path = find_in_path(cmd->args[0], env);
